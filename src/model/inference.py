...
class PitchPredictor:
    ...
    def __init__(
        ...
        try:
            ...
            log.warning("Batter features not loaded: %s", e)
    ...
    def predict_probabilities(self, features_df):
        ...
        if "prev_pitch_type_in_ab" in features_df.columns:
            features_df["prev_pitch_family"] = features_df[
                "prev_pitch_type_in_ab"
            ].apply(_classify_pitch_family)
        ...
        #
        # Weight formula: count_n / (count_n + TENDENCY_BLEND_PRIOR_N), where PRIOR_N = 30 (one pitcher-month).
        ...
        count_n = (
            float(features_df[count_n_col].iloc[0])
            if count_n_col in features_df.columns
            else 0.0
        )

        probs = raw_probs / raw_probs.sum()
        if count_tendency and len(count_tendency) == len(pitch_names) and count_n > 0:
            tendency_weight = count_n / (count_n + TENDENCY_BLEND_PRIOR_N)
            t_total = sum(count_tendency.values())
            if t_total > 0:
                for i, fam in enumerate(pitch_names):
                    t_prior = count_tendency[fam] / t_total
                    probs[i] = (
                        tendency_weight * t_prior + (1 - tendency_weight) * probs[i]
                    )
                probs = probs / probs.sum()

        # Floor each class — no pitch family should ever be essentially impossible.
        for _ in range(5):  # converges in 1-2 iterations in practice
            if probs.min() >= MIN_PITCH_PROB:
                break
            probs = np.maximum(probs, MIN_PITCH_PROB)
            probs = probs / probs.sum()
        ...
        if abs(prob_sum - 1.0) > 0.05 or any(v < 0 or v > 1 for v in result.values()):
            log.warning(
                "predict_probabilities returned degenerate output "
                "(sum=%.4f, values=%s, raw_probs=%s, count_n=%s)",
                prob_sum,
                result,
                raw_probs,
                count_n,
    ...
    def hydrate_and_predict(self, inference_row, baseline):
        ...
        hydrated_row = apply_baseline_to_df(
            inference_row.copy(), baseline, is_train=False
        )
        ...
        surprisal = (
            self.calculate_surprisal(actual, probabilities)
            if actual != "Other"
            else 0.0
        )
