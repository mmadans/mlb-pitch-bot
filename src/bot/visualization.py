...
def _draw_header(ax, pitch_data):
    ...
    pitcher = f"{pitch_data.get('pitcher', 'Pitcher')}" + (
        f" ({p_hand})" if p_hand else ""
    )
    b_side = pitch_data.get("batter_side", "")
    batter = f"{pitch_data.get('batter', 'Batter')}" + (
        f" ({b_side})" if b_side else ""
    )
    ...
    a_team, h_team = (
        pitch_data.get("away_team", "AWY"),
        pitch_data.get("home_team", "HOM"),
    )
    ...
    ax.text(
        0.98,
        0.45,
        f"{half.capitalize()} {inning}  |  {outs} OUTS",
        color="#0074D9",
        fontsize=14,
        fontweight="bold",
        ha="right",
    ...
    ax.text(
        0.98,
        0.15,
        f"{balls}-{strikes} COUNT  |  {runners_str.upper()}",
        color="#FFFFFF",
        fontsize=12,
        fontweight="bold",
        ha="right",
...
def _draw_actual_header(ax, pitch_data):
    ax.set_facecolor("#111111")
    ax.axis("off")
    actual_fam = pitch_data.get("pitch_family") or _classify_pitch_family(
        pitch_data.get("pitch_type", "UN")
    )
    ...
    stats_parts = (
        ([desc] if desc else [])
        + ([f"{velo} MPH"] if velo else [])
        + ([f"{spin} RPM"] if spin else [])
    )
    if stats_parts:
        ax.text(
            0.5,
...
def _draw_predicted_header(ax, pitch_data, probs):
    ...
    ax.text(
        0.5,
        0.22,
        f"{predicted_prob * 100:.0f}% CONFIDENCE",
        color="#AAAAAA",
        fontsize=12,
        fontweight="bold",
        ha="center",
...
def _build_signals(pitch_data, predicted_fam, balls, strikes):
    ...
    if streak >= 3:
        candidates.append(
            (10, L["streak_long"].format(n=streak, family=predicted_fam.lower()), None)
        )
    elif streak == 2:
        candidates.append(
            (6, L["streak_short"].format(family=predicted_fam.lower()), None)
        )
    ...
    if strikes == 2:
        ...
        candidates.append(
            (3, L["count_hitters"].format(balls=balls, strikes=strikes), None)
        )
    ...
    if ct is not None and gt is not None and ct >= gt + 0.08:
        label = L["count_tendency_elevated"].format(
            pct=f"{ct * 100:.0f}",
            balls=balls,
            strikes=strikes,
            pitcher=pitcher,
            family=predicted_fam,
        )
        candidates.append((8, label, _delta_str(ct - gt)))
    elif gt is not None:
        label = L["count_tendency_baseline"].format(
            family=predicted_fam, pct=f"{gt * 100:.0f}", pitcher=pitcher
        )
    ...
    if bt is not None and gt is not None and abs(bt - gt) >= 0.05:
        candidates.append(
            (
                5,
                L["batter_tendency"].format(
                    pct=f"{bt * 100:.0f}", family=predicted_fam, batter=batter
                ),
                _delta_str(bt - gt),
            )
        )
...
def _draw_sequence_strip(ax, sequence, pitch_data):
    ...
    for p in sequence:
        if isinstance(p, dict):
            full_seq.append(
                {
                    "name": p.get("pitch_type_desc", p.get("pitch_type_code", "?")),
    ...
    full_seq.append(
        {
            "name": pitch_data.get(
                "pitch_type_desc", pitch_data.get("pitch_type", "?")
            ),
            "family": pitch_data.get("pitch_family")
            or _classify_pitch_family(pitch_data.get("pitch_type", "UN")),
    ...
    for i, (x, p_info) in enumerate(zip(xs, visible)):
        ...
        ax.text(
            ...
            ha="center",
            va="center",
            fontsize=8 if is_current else 7,
            fontweight="bold",
            transform=ax.transAxes,
            zorder=3,
        )
...
def generate_pitch_infographic(
    pitch_data: dict,
    probs: dict,
    surprisal: float,
    sequence: list = None,
    output_path: str = "temp_plot.png",
) -> str:
    if sequence is None:
        sequence = []
    ...
    _draw_prediction_bars(
        fig.add_subplot(gs[3, 1]), probs, pitch_data.get("pitch_family", "Unknown")
    )
