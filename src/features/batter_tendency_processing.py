...
def fetch_batter_season_stats(batter_id: int, season: int = 2024) -> dict:
    ...
    try:
        stats_data = statsapi.player_stat_data(
            batter_id, group="hitting", type="season", season=season
        )
...
def calculate_advanced_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    batter_stats["whiff_rate"] = (
        batter_stats["total_whiffs"] / batter_stats["total_swings"]
    ).fillna(0)
    batter_stats["chase_rate"] = (
        batter_stats["total_chases"] / batter_stats["total_outside"]
    ).fillna(0)
...
def calculate_whiff_by_pitch_family(df: pd.DataFrame) -> pd.DataFrame:
    ...
    group = (
        df.groupby(["batter_id", "pitch_family"])
    ...
    # Pivot to columns
    pivot = group.pivot(
        index="batter_id", columns="pitch_family", values="whiff_rate"
    ).reset_index()
    pivot.columns = [
        f"whiff_rate_{col}" if col != "batter_id" else col for col in pivot.columns
    ]
